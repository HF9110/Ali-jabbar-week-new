...
// دالة قراءة متغيرات البيئة بأمان
...
const AdminAuthModal = ({ isOpen, onClose, onAuthSuccess }) => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // عند النجاح، سيتم استدعاء onAuthSuccess
      // ملاحظة: سيقوم useEffect في المكون الرئيسي أيضاً بإغلاق النافذة بناءً على حالة المستخدم
      onAuthSuccess();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <GlassCard isGlassmorphism className="w-full max-w-sm" color="bg-gray-900" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <button type="submit" disabled={isLoading} className="w-full p-3 rounded-lg font-bold text-lg text-gray-900 transition duration-300 disabled:opacity-50" style={{ backgroundColor: `var(--main-color-css)` }}>
  ...
  return (
    <GlassCard isGlassmorphism={settings.useGlassmorphism} color="bg-gray-900" className="flex flex-col h-full overflow-hidden hover:shadow-highlight transition duration-300 cursor-pointer">
      <div className="relative overflow-hidden w-full aspect-[2/3] rounded-lg mb-3" onClick={() => onOpenVideo(submission)}>
        <img src={thumbnailUrl} alt={`Thumbnail for ${participantName}`} className="w-full h-full object-cover transition duration-300 transform hover:scale-105" onError={(e) => (e.target.src = 'https://placehold.co/600x900/6b7280/ffffff?text=Video')} />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm" onClick={onClose}>
      <GlassCard isGlassmorphism={settings.useGlassmorphism} color="bg-gray-900" className="w-full max-w-xl max-h-[95vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-end items-center mb-3">
          <button onClick={onClose} className="text-white hover:text-highlight-color transition"><X className="w-8 h-8" /></button>
        </div>
  ...
  const CompactPodiumItem = ({ submission, rank, settings }) => {
    ...
    return (
      <div className="relative flex flex-col items-center p-3 text-center w-1/3 transform hover:scale-105 transition duration-300 rounded-lg" style={{ backgroundColor: `${rankColor}30`, border: `2px solid ${rankColor}`, boxShadow: `0 0 10px ${rankColor}80` }}>
        ...
        <img src={thumbnailUrl} alt={`Rank ${rank}`} className="w-12 h-18 object-cover rounded-md mb-2 border-2" style={{ borderColor: rankColor }} />
  ...
  return (
    <GlassCard isGlassmorphism={settings.useGlassmorphism} color="bg-gray-800" className="p-4 mb-6 shadow-2xl">
      ...
      <div className="flex justify-around gap-2 mb-6">
        {topThree.map((sub, index) => <div key={sub.id} className="w-1/3"><CompactPodiumItem submission={sub} rank={index + 1} settings={settings} /></div>)}
      </div>
        ...
        <div className="space-y-6">
          ...
          <GlassCard isGlassmorphism={settings.useGlassmorphism} color="bg-gray-800" className="p-4 flex flex-col md:flex-row gap-4">
            <div className="relative w-full md:w-1/2">
              <input type="text" placeholder="البحث باسم المشارك أو البلد..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="w-full p-3 pr-10 rounded-lg bg-gray-900/80 border border-white/10 text-white focus:ring-highlight-color focus:border-highlight-color transition duration-300" />
            ...
            <div className="w-full md:w-1/2 flex items-center justify-end text-white">
              <span className="text-lg font-semibold ml-2">إجمالي المشاركات:</span><span className="text-2xl font-extrabold" style={{ color: `var(--highlight-color-css)` }}>{totalApproved.toLocaleString()}</span>
            </div>
          </GlassCard>
          ...
          {filteredSubmissions.length === 0 ? (<p className="text-white/70 text-center text-xl mt-10">لا توجد مشاركات مطابقة لمعايير البحث.</p>) : (
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {currentSubmissions.map((sub) => (<ContestCard key={sub.id} submission={sub} settings={settings} onVote={handleVoteFromCard} onOpenVideo={handleOpenVideo} />))}
              </div>
              {totalPages > 1 && (
                <div className="flex justify-center items-center space-x-4 mt-8">
                  <button onClick={() => setCurrentPage((prev) => Math.max(1, prev - 1))} disabled={currentPage === 1} className="p-3 rounded-lg bg-white/10 text-white disabled:opacity-30 hover:bg-white/20 transition"><ChevronRight className="w-5 h-5" /></button>
                  <span className="text-white text-lg font-semibold">صفحة {currentPage} من {totalPages}</span>
                  <button onClick={() => setCurrentPage((prev) => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} className="p-3 rounded-lg bg-white/10 text-white disabled:opacity-30 hover:bg-white/20 transition"><ChevronLeft className="w-5 h-5" /></button>
                </div>
              )}
            </>
          )}
      ...
      <Modal isOpen={isOpen} onClose={onClose} title="تعديل المشاركة">
        <div className="space-y-4">
          ...
          <div className="flex justify-end pt-4">
            <button onClick={handleSave} className="py-2 px-6 rounded-lg text-gray-900 font-semibold transition" style={{ backgroundColor: settings.mainColor }}>حفظ التعديلات</button>
          </div>
        </div>
      </Modal>
    );
  ...
  return (
    <GlassCard isGlassmorphism={isGlassmorphism} color="bg-gray-900" className="p-6 mb-6">
      ...
      <div className="max-h-96 overflow-y-auto custom-scrollbar">
        {filteredSubmissions.length > 0 ? (filteredSubmissions.map((sub) => <SubmissionRow key={sub.id} sub={sub} />)) : (<p className="text-white/50 text-center py-4">لا توجد مشاركات في هذه الفئة حالياً.</p>)}
      </div>
  ...
  const ColorPicker = ({ label, value, onChange }) => (
    <div className="flex flex-col">
      ...
      <div className="flex items-center gap-2">
        <input type="color" value={value} onChange={(e) => onChange(e.target.value)} className="w-8 h-8 rounded-md p-0 border-none cursor-pointer" />
        <input type="text" value={value} onChange={(e) => onChange(e.target.value)} className="p-2 rounded-lg bg-gray-800/80 border border-white/20 text-white text-sm w-24" />
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div className="flex items-center pt-2">
            <input type="checkbox" id="glassmorphism" checked={currentSettings.useGlassmorphism} onChange={(e) => handleChange('useGlassmorphism', e.target.checked)} className="h-5 w-5 rounded border-gray-300 text-main-color focus:ring-main-color" style={{ backgroundColor: currentSettings.mainColor, borderColor: currentSettings.mainColor }} />
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {Object.keys(STAGES).map((stageKey) => (
                <button key={stageKey} onClick={() => handleChange('stage', stageKey)} className={`py-2 px-4 rounded-lg text-sm font-semibold transition ${currentSettings.stage === stageKey ? 'text-gray-900 shadow-lg' : 'bg-gray-700/70 text-white/80 hover:bg-gray-600/70'}`} style={{ backgroundColor: currentSettings.stage === stageKey ? currentSettings.mainColor : undefined }}>
    ...
    <div className="container mx-auto flex justify-between items-center">
      ...
      <nav className="flex items-center space-x-6 space-x-reverse text-white">
         {(currentStage === 'Voting' || currentStage === 'Ended') && (
             <a href="#submission" className="font-semibold hover:opacity-80 transition py-2 px-4 rounded-full text-white" style={{ backgroundColor: `var(--main-color-css)`, boxShadow: `0 0 10px var(--main-color-css)` }}>إرسال مشاركة جديدة</a>
          )}
      </nav>
    </div>
  </header>
        ...
        <div className="space-y-4">
          {ORGANIZERS.map((org, index) => (
            <div key={index} className="flex items-center bg-gray-800/80 p-3 rounded-lg border border-white/10">
              ...
              <div className="text-right"><p className="text-lg font-bold text-white">{org.name}</p><p className="text-sm font-medium" style={{ color: settings.mainColor }}>{org.role}</p><p className="text-xs text-white/70 dir-ltr text-right">{org.tiktok}</p></div>
            </div>
          ))}
        </div>
      </Modal>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (urlParams.has('admin') && urlParams.get('admin') === 'true') {
      if (auth?.currentUser) setAdminMode(true);
      else setAuthModalOpen(true);
    }
  }, [isAuthReady]);

  // Auto-close modal and switch to admin mode upon successful login
  useEffect(() => {
    if (userId && userId !== 'public-read-only') {
       const urlParams = new URLSearchParams(window.location.search);
  ...
  useEffect(() => {
    ...
    const initializeFirestore = async () => {
      ...
      } catch (e) { setError('Failed to initialize data. Check permissions or Firebase configuration.'); }
      setLoading(false);
    };
    initializeFirestore();
  }, [isAuthReady]);
  ...
  useEffect(() => {
    ...
    const unsubscribeSettings = onSnapshot(settingsDocRef, (docSnap) => {
        ...
        setLoading(false);
      }, (e) => { setError('Failed to load settings. Check Firestore connectivity.'); setSettings(DEFAULT_SETTINGS); setLoading(false); }
    );
  ...
  const handleAdminLoginSuccess = () => { setAdminMode(true); setAuthModalOpen(false); };
  ...
  const handleSaveSettings = async (newSettings) => { try { if (!db) return; const settingsDocRef = doc(db, PUBLIC_SETTINGS_PATH); await retryOperation(() => setDoc(settingsDocRef, newSettings)); } catch (e) { console.error(`فشل حفظ الإعدادات: ${e.message}`); } };
  const handleUpdateSubmissionStatus = async (id, newStatus) => { try { if (!db) return; const docRef = doc(db, PUBLIC_SUBMISSIONS_COLLECTION, id); await retryOperation(() => updateDoc(docRef, { status: newStatus })); } catch (e) { console.error(`فشل تحديث حالة المشاركة: ${e.message}`); } };
  const handleConfirmVote = async (submission) => {
    ...
    try { if (!db) return; const docRef = doc(db, PUBLIC_SUBMISSIONS_COLLECTION, submission.id); await retryOperation(() => updateDoc(docRef, { votes: increment(1) })); setCooldown(30); } catch (e) { console.error(`فشل التصويت: ${e.message}`); }
  };
  const handleVote = (submission) => { if (cooldown > 0) { console.warn(`الرجاء الانتظار ${cooldown} ثواني قبل التصويت مرة أخرى.`); return; } setVoteConfirmData(submission); };
